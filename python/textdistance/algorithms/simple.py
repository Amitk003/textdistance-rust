...
class Prefix(_BaseSimilarity):
    ...
    def __call__(self, *sequences: Sequence) -> Sequence:
        ...
        test_func = None if self.sim_test is self._ident else self.sim_test
        result = _textdistance.common_prefix(sequences, test_func)
...
class Length(_Base):
    ...
    def __call__(self, *sequences: Sequence) -> int:
        return _textdistance.length(sequences)
